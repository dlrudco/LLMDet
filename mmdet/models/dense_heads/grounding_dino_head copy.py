...
class GroundingDINOHead(DINOHead):
    ...
    def predict(self,
        ...
        head_pre_end = forward_start = time.time()
        self.head_time_profiles.setdefault('pre', 0)
        self.head_time_profiles['pre'] += head_pre_end - head_start
        outs = self(hidden_states, references, memory_text, text_token_mask)
        forward_end = post_start = time.time()
        self.head_time_profiles.setdefault('forward', 0)
        self.head_time_profiles['forward'] += forward_end - forward_start
        predictions = self.predict_by_feat(
            *outs,
            ...
            rescale=rescale)
        post_end = time.time()
        self.head_time_profiles.setdefault('post', 0)
        self.head_time_profiles['post'] += post_end - post_start
        self.head_time_profiles.setdefault('total', 0)
        self.head_time_profiles['total'] += post_end - head_start
        print({key: round(100* self.head_time_profiles[key] / self.head_time_profiles['total'], 2)
                    for key in self.head_time_profiles})
        return predictions
